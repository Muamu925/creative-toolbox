...
**当前为 0.8.0 早期预览版**，已提供首页、工具搜索与收藏，以及智能保存、字体管理与对照、创作换算和色彩工具。图片库支持集合归类、来源整理、单图置顶、备份恢复和关联提色；新增离线帮助中心。多图参考板、项目管理、批量交付与音视频处理尚未实现。

[**下载 Windows 0.8.0 预览版**](https://github.com/Muamu925/creative-toolbox/releases/download/v0.8.0/CreativeToolbox-0.8.0-Windows-x64.zip) · [所有下载 / macOS 实验版](https://github.com/Muamu925/creative-toolbox/releases/tag/v0.8.0) · [English](README.en.md) · [功能路线](docs/FEATURE_ROADMAP.md)

## 0.8.0：一起找回，一起带走

在「资源库」同时搜索图片、色板和字体整理记录，双击结果回到对应工具。完整备份包含当前图片原件、集合、色板、字体整理及已保存设置；恢复先校验到新目录，切换后重新打开工具箱。

不包含系统字体文件、未保存编辑和活动日志；搜索不扫描整盘，也不枚举未整理的系统字体。[使用与恢复说明](docs/RESOURCE_SEARCH_BACKUP.md)

![统一资源搜索，独立示例资料](assets/demo/resource-search.png)
...
| **个人资源库** | 已有跨资源检索和完整备份；下一步连接轻量项目资料与批量归类 |
...
当前有 156 项自动化测试，覆盖保存规则、字体整理、配色持久化、悬浮同步、换算、工具入口与故障降级，以及素材收集、集合升级、关联色板、帮助导航、备份校验和独立恢复；另覆盖外观持久化、图标与文字对比度。自动化测试不能代替所有创作软件的真实兼容性验证。
