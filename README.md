...
**当前为 0.4.0 早期预览版**，已提供首页、工具搜索与收藏，以及智能保存、字体管理与对照、创作换算和色彩工具。参考图、项目管理、批量交付与音视频处理正在规划，尚未实现。

[**下载 Windows 0.4.0 预览版**](https://github.com/Muamu925/creative-toolbox/releases/download/v0.4.0/CreativeToolbox-0.4.0-Windows-x64.zip) · [所有下载 / macOS 实验版](https://github.com/Muamu925/creative-toolbox/releases/tag/v0.4.0) · [English](README.en.md) · [功能路线](docs/FEATURE_ROADMAP.md)

## 现在有哪些工具

| 模块 | 已实现的能力 | 使用场景 |
| --- | --- | --- |
| **首页与工具入口** | 工具搜索、收藏、最近使用、字体与配色资源入口；页面按需加载 | 快速找到工具并继续上次工作 |
...
这些工具可以独立使用，不要求先建项目或开启自动保存。工具箱不需要账号，配色库、应用规则和日志保存在本机，图片提色不上传图片。当前界面以中文为主。
...
![0.4.0 首页与常用工具](assets/demo/workspace-home.png)

新入口与数据说明见 [首页与工具使用指南](docs/WORKSPACE.md)。资源库当前集中字体与配色入口，图片收集箱尚未实现。下方字体与色彩截图来自此前版本，工具能力仍保留。
...
| **个人资源库** | 图片收集箱、集合与标签、来源记录、置顶参考；连接个人配色和字体资料 |
...
入口与基础第一版已完成；下一步是 **积累个人资料 → 组合项目与交付**。这些模块将共享资源引用与项目规格，同时保留直接打开小工具的方式。实施顺序、架构与验收见 [整体架构与发展方案](docs/ARCHITECTURE_PLAN.md)；全部候选能力见 [功能地图](docs/FEATURE_ROADMAP.md)。
...
当前有 81 项自动化测试，覆盖保存规则、字体整理、配色持久化、悬浮同步、换算、工具入口与故障降级。自动化测试不能代替所有创作软件的真实兼容性验证。
