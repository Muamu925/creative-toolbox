...
**当前为 0.3.0 早期预览版**，已提供智能保存、字体管理与对照、创作换算和色彩工具。参考图、项目管理、批量交付与音视频处理正在规划，尚未实现。

[**下载 Windows 0.3.0 预览版**](https://github.com/Muamu925/creative-toolbox/releases/download/v0.3.0/CreativeToolbox-0.3.0-Windows-x64.zip) · [所有下载 / macOS 实验版](https://github.com/Muamu925/creative-toolbox/releases/tag/v0.3.0) · [English](README.en.md) · [功能路线](docs/FEATURE_ROADMAP.md)
...
2. 按当前任务进入「字体工作台」「创作换算」或「配色工作台」，输入文案、数值或图片即可开始使用。
3. 如需智能保存，先在「应用规则」配置目标软件与快捷键，通过「智能保存」的观察模式检查触发时机，再用可丢弃文件试用。
...
| 方向 | 下一步构思 |
| --- | --- |
| **统一工具入口** | 工具搜索、收藏、最近使用，按设计 / 视频 / 音乐场景组织常用工具 |
...
发展顺序是：**整理入口与基础 → 积累个人资料 → 组合项目与交付**。这些模块将共享资源引用与项目规格，同时保留直接打开小工具的方式。实施顺序、架构与验收见 [整体架构与发展方案](docs/ARCHITECTURE_PLAN.md)；全部候选能力见 [功能地图](docs/FEATURE_ROADMAP.md)。
...
当前有 66 项自动化测试，覆盖保存规则、配色持久化、悬浮同步、撤销、换算和 PNG 导出。自动化测试不能代替所有创作软件的真实兼容性验证。
