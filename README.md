...
面向设计、剪辑、三维和音乐软件的本地桌面工具箱。当前版本 **0.1.0** 包含通用智能保存，以及配色工作台、创作换算和字体对照，使用 Python + PySide6。
...
36 项自动化测试已通过，覆盖 Windows 原生状态读取、输入包构造、核心规则、配色库、颜色与尺寸计算和 Qt 界面；Windows 独立程序已通过启动与自身界面渲染检查。真实跨进程 Ctrl+S 的隔离测试中，系统曾接受输入，但未取得接收窗口确认；后续尝试又遇到前台焦点被切换，测试按规则取消。**目前不把真实编辑软件的自动保存标为已验证**，应先在可丢弃测试文件中试用。
