...
TOPICS = (
    Topic("library", "统一查找资料", "一次搜索，找回图片、色板和字体整理。", "", (
        ("怎样搜索？", "在侧栏「资源库」输入名称、标签或色号，可按图片、色板、字体筛选。空格分隔的关键词需要同时匹配。每类最多展示 100 条，双击、回车或点击打开按钮进入原工具。"),
        ("搜哪些内容？", "图片搜索名称、标签、来源和备注；色板搜索名称、颜色名称、HEX 和来源图名；字体搜索已保存记录的家族名、别名、分组、标签、来源与备注。不会扫描整盘，也不枚举所有未整理的系统字体。回收站图片不进入搜索结果。"),
        ("没有搜到？", "检查是否已保存整理或色板。资源损坏时会显示对应模块的提示，其余模块仍可搜索。色板在搜索后发生变化时，请重新搜索。当前不可用的字体仍可打开整理记录。"),
        ("带走全部资料", "同一页面的「完整备份」保存图片原件、集合、色板、字体整理和已保存设置；「恢复备份」先在新目录校验，完成后可切换并重新打开工具箱。详见数据、备份与恢复。"),
    )),
    Topic("start", "第一次使用", "用一个小任务开始，无需账号或项目。", "assets", (
        ("先试哪一个？", "收集参考：打开图片素材库，导入一张 PNG / JPEG。\n选择字体：打开字体库，输入自己的标题，选 2–4 个字体比较。\n取用颜色：打开色板，点击色块复制色号。\n计算尺寸：打开创作换算，输入毫米与 PPI，查看像素结果。"),
    ...
    Topic("assets", "图片素材库", "收集 → 整理 → 找回 → 对照参考。", "assets", (
        ("导入与粘贴", "从首页「收集图片」进入。点击导入、拖入本地 PNG / JPEG，或点击「粘贴图片」。不会自动读取剪贴板。图片复制到本地素材库，原文件移动后仍可使用。重复收集保留独立记录，相同原件共用存储。"),
    ...
    Topic("palettes", "配色与图片来源", "保存自己的颜色，并知道它们从哪里来。", "palettes", (
        ...
        ("文字对比度怎么算？", "按 WCAG 2.2 的不透明 sRGB 颜色计算。大号文字指至少 18 pt，或至少 14 pt 的粗体。只检查颜色对比，不代表整体无障碍合规。"),
    ...
    Topic("fonts", "字体库", "整理系统可用字体，比较自己的文案。", "fonts", (
    ...
    Topic("calculators", "尺寸与节奏换算", "快速计算常用数值。", "calculators", (
        ("毫米、像素与 PPI", "选择转换方向，填写宽高与 PPI。字段单位随方向切换。结果取整用于尺寸估算，不含出血，不改变图片或文档设置。PPI 是每英寸像素数，和打印机 DPI 不同。"),
    ...
    Topic("backup", "数据、备份与恢复", "知道哪些数据被保存，以及怎样带走。", "assets", (
        ...
        ("完整备份", "在「资源库」点击「完整备份」，保存 .ctbackup 文件。包含当前图片库的原件、集合、回收站，以及色板、字体整理、已保存应用规则、工具收藏与外观设置；不包括系统字体文件、未保存编辑、活动日志和旧恢复目录。处理期间暂停编辑，并切回观察模式。"),
        ("恢复与切换", "点击「恢复备份」，选择 .ctbackup 文件。程序会校验内容、重建图片预览并恢复到新目录，保留原库和图片与色板的来源关联。成功后选择切换，工具箱退出；重新打开便使用恢复的资料。字体仍需在目标电脑上安装。"),
        ("返回切换前的资料", "先退出工具箱并备份数据目录。在最初的数据目录中，将 active-workspace.json 改名保留，重新打开会使用最初的资料。该文件记录当前恢复目录，可留存以便返回。不要在运行时移动目录。"),
    ...
    Topic("settings", "窗口、设置与常见问题", "处理运行、数据和反馈问题。", "", (
        ("如何关闭动效？", "在设置的「外观」中开启「减少动态效果」。页面和导航立即切换，复制与保存提示仍会显示。选择保存在本机。"),
