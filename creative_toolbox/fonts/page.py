...
class FontPage(QWidget):
    def __init__(self, path=None, available=None):
        ...
        self.system.setAccessibleName("文字系统筛选")
        ...
        self.batch_button = button("执行", self.batch_action)
        self.details_button = button("详情 / 备注", self.edit_details)
        self.details_button.setToolTip("选择一款字体后查看详情与备注")
        batch.addWidget(self.batch_button)
        batch.addWidget(self.details_button)
        batch.addStretch()
        self.compare_button = button("对照所选（2–4）", self.compare_selected)
        self.compare_button.setObjectName("primary")
        self.compare_button.setToolTip("按 Ctrl / Command 选择 2–4 款本机可用字体")
        batch.addWidget(self.compare_button)
    ...
    def update_count(self, *args):
        ...
        self.batch_button.setEnabled(bool(selected))
        self.details_button.setEnabled(len(selected) == 1)
        self.compare_button.setEnabled(2 <= len(selected) <= 4 and
                                      all(f in self.model.available for f in selected))
