...
实施更新（2026-09-22）：0.6.0 增加多集合归类、图片生成色板与反向来源跳转、离线帮助中心。来源绑定库 ID / 素材 ID / 摘要，不依赖色板数组位置；尚未引入项目引用所需的色板稳定 ID。B 阶段仍有跨资源检索与统一备份待完成。

实施更新（2026-09-26）：0.8.0 提供图片、色板、字体整理的后台统一搜索，以及完整工作空间备份、校验恢复和重启切换。现有存储保持不变；色板搜索跳转使用内容指纹防止过期结果错连，并不代替项目所需的稳定资源 ID。详见 [搜索与完整备份](RESOURCE_SEARCH_BACKUP.md)。
