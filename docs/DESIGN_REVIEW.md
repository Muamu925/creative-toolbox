# 2026-09-22 · 0.7.1 动效与文案

140 项测试通过，新增 10 项覆盖快速切页、中途减少动效、缩放取消、导航跟随尺寸、轻提示替换与焦点、旧外观设置兼容、无效设置保留、复制结果及保存失败不误报成功。原有保存和资源功能测试保留。

原生录制检查了导航、切页、复制结果和关闭动效；发现并修复了过渡快照灰底闪变。提示收紧为随内容长度变化。主要页面在 1180×800 与 1020×720 逻辑尺寸检查，动画等待完成后再生成静态截图。动态演示在 assets/demo/interaction-demo.gif；本地原始帧与时间记录在 artifacts/motion-frames。

文字采用 UX Writing 原则：任务名称替代宣传句，动作替代泛称，计算原理收进帮助。必要的保存状态、未保存错误与字体回退说明保留。尚未进行真实用户访谈或屏幕阅读器实测。
