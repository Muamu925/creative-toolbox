...
- 设计用户可尝试字体管理与对照或尺寸换算；音乐用户可尝试 BPM 音符时长；色彩工具作为另一条独立体验。视频用户可试用现有通用换算并反馈需求，当前尚无专用媒体处理。
...
当前 0.3.0 预览版已经有智能保存规则、字体管理与对照、尺寸与 BPM 换算，以及悬浮色卡等色彩工具。下一步计划加入参考图画板、工具搜索、文件整理和更多音视频辅助功能。
...
下载：https://github.com/Muamu925/creative-toolbox/releases/tag/v0.3.0
...
The 0.3.0 preview includes per-app idle-save rules, font organization and comparison, size and BPM calculators, and color tools. Reference boards, tool search, file organization and more media utilities are planned additions.
...
开发环境下设置 PYTHONPATH 为仓库根目录，运行 `python tools/capture_fonts.py`。脚本使用 Windows 本机字体和临时整理数据，保存原生字体库与对照截图，不改变系统字体。
