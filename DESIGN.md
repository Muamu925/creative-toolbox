...
设置 → 外观 → 减少透明效果，立即切换为不透明导航并移除背景光晕。偏好独立保存到 `appearance.json`，不影响保存规则或资源数据；损坏/未来格式保留原文件，只在本次窗口应用调整。写入失败必须明确提示。材质本身无动画；界面没有循环播放的装饰动效。
...
本轮通过 getdesign 0.6.25 添加 [Figma、Notion、Apple 参考](docs/design-references/README.md)。它们是第三方静态视觉分析；以下交互规则是本项目自己的设计决定。

- 页面：目标页立即切换，旧页快照在 160ms 内淡出。快照保留应用背景，不会出现灰底闪变，也不拦截鼠标。连续切换以最后一次操作为准，缩放/隐藏立即清理过渡。
- 导航：相邻选中项 150ms 平滑移动；跨越大段空白时直接定位，避免长距离扫过侧栏。
- 操作结果：确认复制或保存完成后才显示轻提示，130ms 淡入/淡出、停留 2.6s；连续操作替换同一条提示，不排队、不抢焦点。页面原有状态或错误文字仍保留。
- 设置 → 减少动态效果：立即停止过渡，后续页面和导航直接切换；操作结果仍可见。偏好作为可选字段保存在 appearance.json，旧外观文件继续可读。
- 标题直接描述任务，如「图片素材」「字体库」「色板」「尺寸与节奏」；按钮使用「收集图片」「保存快捷键」等动作。说明先回答当前问题，详细计算规则移到帮助，错误与保存边界仍在需要处显示。

实际演示：[页面切换与复制反馈](assets/demo/interaction-demo.gif)。只录制工具箱自身窗口，使用隔离数据与不可发送按键的后端。
